...
public class DetailFragment extends Fragment {
    ...
    // Method called when the user presses the FAB
    private void done() {

        // Make sure that the listener object has been defined correctly
        // If the listener is null ... throw an error
        if (mListener == null) {
            throw new AssertionError();
        }

        // If the code comes down here, I can now send the message to the Activity

        // Get the data from the TextFields that the user has typed in
        String firstName = textFirstName.getText().toString();
        String lastName = textLastName.getText().toString();
        int age = Integer.valueOf(textAge.getText().toString());

        // Call the listener object's method passing in the values
